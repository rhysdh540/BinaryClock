...
public class Panel extends JPanel{
    ...
    public void paint(Graphics g){
        //setup
        this.setBackground(prefs.get("RAVE MODE") ? rave() : (prefs.get("Dark Mode") ? Color.black : Color.white));
        g.setColor(prefs.get("Dark Mode") && !prefs.get("RAVE MODE") ? Color.white : Color.black);
        g.setFont(MONO);
        super.paintComponent(g);

//        while(!SwingUtilities.isDescendingFrom(this, KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusedWindow())) {
...
//        }
        ...
        final boolean[][] lines = new boolean[][]{clock.getHours(), clock.getMinutes(), clock.getSeconds(), null};
        ...
        for (int i = -1; i < 3; i++) {
            for (int j = -3; j < 3; j++) {
                boolean[] line = lines[i + 1]; // decides which line to draw (i==-1: hours, i==0: minutes, i==1: seconds, i==2: not drawing so we don't care)
                ...
                if(line != null) { // draw the clock grid
                    g.setFont(SFPROBIG);
                    if (line[(prefs.get("Flip Binary Clock") ? 5 - (j + 3) : (j + 3))]) { // if the bit is on, fill a circle/draw 1
                    ...
                    g.setFont(SFPRO);
                    int value = (int) Math.pow(2.0, (prefs.get("Flip Binary Clock") ? (j + 3) : 5 - (j + 3)));
                    g.drawString(String.valueOf(value), x + 7, y);
                }
            }
        }
